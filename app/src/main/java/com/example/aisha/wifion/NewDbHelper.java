...
*/
    ...
    private static final String DATABASE_CREATE=        "create table " + TABLE_MESSAGES + "(" + COLUMN_ID +" integer primary key autoincrement, "
            + COLUMN_MESSAGE + " text not null,"
            + COLUMN_ROLLNO + " text not null unique ,"
            + COLUMN_SEM + " text not null"
            + ");";
    ...
    public void onUpgrade(SQLiteDatabase sqLiteDatabase, int oldVersion, int newVersion) {
        Log.w(NewDbHelper.class.getName(), "Upgrading database from version " + oldVersion + " to " + newVersion + ", which will destroy all old data");
